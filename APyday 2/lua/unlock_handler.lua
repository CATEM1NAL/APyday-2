...
function apd2_random_upgrades(count, table_name)
  local WorkingTable = apd2_upgrade_tables[table_name]

  -- Remove upgrades that the player already has
  for i = #WorkingTable, 1, -1 do
    if apd2_data.upgrades[WorkingTable[i]] then
  ...
  for i = 1, count do
    ...
    apd2_data.upgrades[WorkingTable[UpgradeIndex]] = true
    table.insert(tweak_data.skilltree.default_upgrades, WorkingTable[UpgradeIndex])
...
function apd2_random_weapons(count, table_name)
  ...
  -- Remove upgrades that the player already has
  for i = #WorkingTable, 1, -1 do
    if apd2_data.weapons[WorkingTable[i]] then
      table.remove(WorkingTable, i)
    end
  end
  count = math.min(count, #WorkingTable)
  ...
  for i = 1, count do
    ...
    apd2_data.weapons[WorkingTable[WeaponIndex]] = true
    log(APD2FileIdent .. "Added " .. WorkingTable[WeaponIndex] .. " to weapon table")
    table.insert(tweak_data.skilltree.default_upgrades, WorkingTable[WeaponIndex])
...
function apd2_random_armors(count)
  ...
  for i = #ArmorTable, 1, -1 do
    if apd2_data.weapons[ArmorTable[i]] then
  ...
  for i = 1, count do
    ...
    apd2_data.weapons[ArmorTable[ArmorIndex]] = true
    log(APD2FileIdent .. "Added " .. ArmorTable[ArmorIndex] .. " to weapon table")
    table.insert(tweak_data.skilltree.default_upgrades, ArmorTable[ArmorIndex])
...
function apd2_random_deployables(count)
  ...
  for i = #DeployTable, 1, -1 do
    if apd2_data.weapons[DeployTable[i]] then
  ...
  for i = 1, count do
    ...
    apd2_data.weapons[DeployTable[DeployIndex]] = true
    log(APD2FileIdent .. "Added " .. DeployTable[DeployIndex] .. " to weapon table")
    table.insert(tweak_data.skilltree.default_upgrades, DeployTable[DeployIndex])
