...
local APD2ScorePerBag = #apd2_data.game.heists * (1 + apd2_data.x.mutators) * (1 + apd2_data.x.diff) * 100
local APD2ScorePerPackage = #apd2_data.game.heists * (1 + apd2_data.x.mutators) * 100
local APD2ScorePerCash = APD2ScorePerBag / 100
local APD2NextPoint = 100 + (apd2_data.game.score - apd2_data.game.score % 100)
...
Hooks:PostHook(LootManager, "secure", "apd2_bagsecured", function(self)
  if not tweak_data.carry.small_loot[self._global.secured[#self._global.secured].carry_id] then
    ...
    -- log(APD2FileIdent .. "ScorePerCash = " .. APD2ScorePerCash)
    -- log(APD2FileIdent .. APD2NextPoint .. " <= " .. apd2_data.game.score)
